...
def main(config):
    ...
    model1  = UNet(in_chns=3, class_num=1)
    model2  = UNet(in_chns=3, class_num=1)
...
def train_val(config, model1, model2, train_loader, val_loader, criterion):
    ...
    for epoch in range(epochs):
        ...
        for idx, (batch, batch_w_s) in enumerate(source_dataset):
            ...
            pseudo_mask_u1 = (outputs_u1 > config.semi.conf_thresh).float()
            outputs_weak_masked_u1 = outputs_u1 * pseudo_mask_u1
            pseudo_u1 = torch.round(outputs_weak_masked_u1)

            outputs_u2, ulb_features_2 = model2(weak_batch, return_features=True)
            outputs_u2 = torch.sigmoid(outputs_u2)
            pseudo_mask_u2 = (outputs_u2 > config.semi.conf_thresh).float()
            outputs_weak_masked_u2 = outputs_u2 * pseudo_mask_u2
            pseudo_u2 = torch.round(outputs_weak_masked_u2)
            ...
            lb_dis_loss = 1 + calculate_cosine_similarity(lb_features_1, lb_features_2).mean() 
            ulb_dis_loss = 1 + calculate_cosine_similarity(ulb_features_1, ulb_features_2).mean()
        ...
        if avg_dice_1 > avg_dice_2:
            ...
            model = model2
        if avg_dice_val_epoch > max_se:
            torch.save(model.state_dict(), best_model_dir)
            max_se = avg_dice_val_epoch
            best_epoch = epoch
