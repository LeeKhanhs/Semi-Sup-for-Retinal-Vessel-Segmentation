...
from Utils.DMT_loss import ClassMixLoss, compute_classmix, compute_cutmix, compute_ic
...
def main(config):
    ...
    cm_loss_fn = ClassMixLoss() 
    
    model = train_val(config, model, model_teacher, model_teacher2, train_loader, val_loader, criterion, cm_loss_fn)
...
def train_val(config, model, model_teacher, model_teacher2, train_loader, val_loader, criterion, cm_loss_fn):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (batch_l, batch_u) in enumerate(zip(train_loader['l_loader'], train_loader['u_loader'])):
            ...
            label_u = batch_l['label'].cuda().float()
            ...
            if do_class_mix:
                loss = compute_classmix(b, h, w, criterion, cm_loss_fn, model, ema_model, img_l, label_l, img_u, image_u_strong, threshold=0.95)
            if do_cut_mix:
                loss = compute_cutmix(h, w, img_l, label_l, criterion, model, ema_model, img_u, threshold=0.95)

            loss_dc = compute_ic(model, ema_model, img_u, image_u_strong, criterion, label_u, h, w, threshold=0.95)
            ...
            loss_train_sum += total_loss.item() * sup_batch_len
        ...
        if (se_val_epoch + acc_val_epoch + sp_val_epoch)/3 > max_avg:
            torch.save(model.state_dict(), best_model_dir)
            max_avg = (se_val_epoch + acc_val_epoch + sp_val_epoch)/3
            best_epoch = epoch
